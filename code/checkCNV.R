checkCNV <- function(SPACEmapX_obj,
                 ...
                 # observation spots clustering settings
                 ...
                 HMM_report_by=c("subcluster","consensus","spot"),
                 HMM_type=c('i6', 'i3'),
                 HMM_i3_pval=0.05,
                 HMM_i3_use_KS=FALSE,
                 BayesMaxPNormal=0.5,
                 ...
                 analysis_mode=c('subclusters', 'samples', 'spots'), # for filtering and HMM
  ...
  if (skip_past < step_count) {
    ...
    ## require each gene to be present in a min number of spots for ref sets
    
    SPACEmapX_obj <- require_above_min_spots_ref(SPACEmapX_obj, min_spots_per_gene=min_spots_per_gene)
  ...
  if (!is.null(num_ref_groups)) {
    ...
    if (! has_reference_spots(SPACEmapX_obj)) {
      stop("Error, no reference spots defined. Cannot split them into groups as requested")
    }
  ...
  if (! is.na(max_centered_threshold)) {
    ...
    ## Cap values between threshold and -threshold, retaining earlier center_spot_expr_across_chromosome
  ...
  ## #########################################################################
  ## Step: For each spot, smooth the data along chromosome with gene windows
  ...
  flog.info(sprintf("\n\n\tSTEP %02d: Smoothing data per spot by chromosome\n", step_count))
  ...
  ##
  ## Step:
  ## Center spots/observations after smoothing. This helps reduce the
  ...
  if (skip_past < step_count) {
    SPACEmapX_obj <- center_spot_expr_across_chromosome(SPACEmapX_obj, method="median")
  ...
  if (HMM) {
    ...
    if (skip_hmm < 1) {
      ...
      } else if (analysis_mode == 'spots') {
        
        if (HMM_type == 'i6') {
          hmm.SPACEmapX_obj <- predict_CNV_via_HMM_on_indiv_spots(SPACEmapX_obj, t=HMM_transition_prob)
        } else if (HMM_type == 'i3') {
          hmm.SPACEmapX_obj <- i3HMM_predict_CNV_via_HMM_on_indiv_spots(SPACEmapX_obj,
                                                                       i3_p_val=HMM_i3_pval,
                                                                       t=HMM_transition_prob,
                                                                       use_KS=HMM_i3_use_KS)
        } else {
